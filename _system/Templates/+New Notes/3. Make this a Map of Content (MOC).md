...
if (file) {
    ...
    if (currentType !== "moc") {
        ...
        if (topicTag) {
            ...
            // Get current content to preserve everything except frontmatter
            ...
            // Get everything after the frontmatter (preserve all existing content)
            const existingContent = frontmatterMatch 
                ? currentContent.substring(frontmatterMatch[0].length).trim() 
                : currentContent.trim();
            
            // Build existing content section if there's any content
...
`;
            ...
            // Combine: frontmatter + preserved content + MOC content
            await app.vault.modify(file, frontmatter + preservedSection + mocContent);
