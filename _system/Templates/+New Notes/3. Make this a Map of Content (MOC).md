...
// Get all Readwise files
const allReadwise = dv.pages('"Inputs/Readwise"')
  .where(p => p.class === "Sources");

// Filter by matching tags
const taggedFiles = allReadwise.where(p => 
  p.file.tags && mocTags.some(mt => p.file.tags.some(t => String(t).includes(String(mt))))
);

// Separate highlights files (don't end with FULL_DOC or FULL DOC) from full doc files
const highlightFiles = taggedFiles.where(p => 
  !p.file.name.includes("FULL_DOC") && !p.file.name.includes("FULL DOC")
);

const fullDocFiles = taggedFiles.where(p => 
  p.file.name.includes("FULL_DOC") || p.file.name.includes("FULL DOC")
);

// For each highlight file, we know there might be a corresponding full doc
// We want: all highlight files + only full doc files where no highlight exists
const highlightTitles = highlightFiles.map(p => p.sourceTitle);

const orphanFullDocs = fullDocFiles.where(p => 
  !highlightTitles.includes(p.sourceTitle)
);

// Combine: highlights (preferred) + orphan full docs (only when no highlights)
const finalList = [...highlightFiles.array(), ...orphanFullDocs.array()];

if (finalList.length > 0) {
  dv.table(
    ["Author", "Title", "Imported"],
    finalList
      .sort((a, b) => (b.date_saved || "").localeCompare(a.date_saved || ""))
      .map(p => [
        p.author || "",
        dv.fileLink(p.file.path, false, p.sourceTitle || p.file.name),
        p.date_saved || ""
      ])
  );
} else {
  dv.paragraph("No matching Readwise items found");
}
${bt3}
...
Captured quotes on this topic from tagged sources.

${bt3}dataviewjs
const mocTags = dv.current().file.tags;

// Get all quotes
const allQuotes = dv.pages('"Synthesis/Quotes"')
  .where(p => p.class === "Quotes");

// Filter quotes that either:
// 1. Have matching tags themselves
// 2. Come from sources that have matching tags
const matchingQuotes = allQuotes
  .where(q => {
    // Check if quote has matching tags
    if (q.file.tags && mocTags.some(mt => q.file.tags.some(t => String(t).includes(String(mt))))) {
      return true;
    }
    
    // Check if source has matching tags
    if (q.source) {
      const sourceMatch = String(q.source).match(/\\[\\[.*?([^\\/\\|]+?)(?:\\|.*?)?\\]\\]/);
      if (sourceMatch) {
        const sourceName = sourceMatch[1].replace('.md', '');
        const sourcePage = dv.page(sourceName);
        if (sourcePage && sourcePage.file.tags) {
          return mocTags.some(mt => sourcePage.file.tags.some(t => String(t).includes(String(mt))));
        }
      }
    }
...
if (matchingQuotes.length > 0) {
  dv.table(
    ...
    matchingQuotes
      ...
      .map(p => {
        // Extract clean source name from the link
        let sourceDisplay = "";
        if (p.source) {
          const sourceMatch = String(p.source).match(/\\[\\[.*?([^\\/\\|]+?)(?:\\|.*?)?\\]\\]/);
          sourceDisplay = sourceMatch ? sourceMatch[1].replace('.md', '') : String(p.source);
        }
        
        // Create clickable link with preview text as display
        const quoteLink = dv.fileLink(p.file.path, false, p.title || p.file.name);
  ...
  dv.paragraph("No matching quotes found");
}
${bt3}
...
*Use [[⌃ ⌘ L - Log Tool]] to add tasks. These will show up under [[2. Reminders]] in Navigation*  
