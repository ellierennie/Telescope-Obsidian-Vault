...
function formatQuoteForInsertion(content, source, quoteFile, isKOI = false, metadata = null) {
    // Format content as italics (wrap in *)
    const italicContent = `*${content}*`;
    ...
    if (isKOI) {
        ...
        return `${italicContent} ([[${quoteFilePath}|${authorName} in ${channelName}]])`;
    } 
    // For non-KOI items, include both source and quote file links
        ...
        return `${italicContent} ([[${cleanSource}|${sourceTitle}]], [[${quoteFilePath}|see quote]])`;
    }
}

// Function to get all quotes directly from Synthesis/Quotes folder (no subfolders)
async function getAllQuotes() {
    ...
    // Get all markdown files directly from the quotes folder
    const quoteFiles = quotesFolder.children.filter(file => file instanceof tp.obsidian.TFile && file.extension === "md");
    const allQuotes = [];
